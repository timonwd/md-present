...
A --&gt; B
	...
	for _, asset := range []string{"app.js", "style.css", "favicon.svg", "mermaid.min.js", "mermaid.LICENSE.txt"} {
		...
		if asset == "app.js" {
			...
			for _, expected := range []string{"requestFullscreen", "exitFullscreen", `securityLevel: "strict"`, `role", "alert"`, "language-mermaid", "mermaidConfigurationDirective", "Mermaid configuration directives are not supported.", "overflowWarningDuration", "checkForUpdate", "/api/update", "scrollBy", "toggleOverview", "closePresentationTab", "window.close()", `role", "grid"`, `role", "gridcell"`, "isMediaControl", "video, audio", "prepareVideoPoster", "captureFirstFrame", "video.currentTime", "canvas.toDataURL"} {
